...
public:
	...
	AxisSettings(int _stepperId, int _hallSensorPin, int _pulsePin, int _directionPin, int _enablePin, int _totalWorkingRange, bool _reverseDirection, int _centerPositionPercent) {
...
public:
	...
	void loop() {
		...
		if (axisState.queuedPosition != -1 && !stepper->isMotorRunning()) {
