...
byte tmin[num_pwm], tmax[num_pwm], *LUT=NULL;   // e.g. per-channel min, max limits
uint16_t minpw[num_pwm], maxpw[num_pwm];
byte gain[num_pwm], spare[num_pwm];             // room to grow
#define NL 3                                    // base 0 LUT count
byte* table[NL+1] = {tmin, tmax, spare};        // subtract 5 from special to index table[]
byte active_PWMs = 4, Lcount = 0, Lidx = 0, Lid = 0;
Servo servo[num_pwm];
byte min_defaults[] = {52,52,52,52,52,52,52,52,52,52,52,52,52,52,52};  // initial servo min settings, where 126 == MAX_PULSE_WIDTH / 2
...
// 0 <= pw <= 126
uint16_t rescalePulseWidth(uint32_t pw, uint16_t minpw, uint16_t maxpw)  // round Pulse widths to nearest microsecond
{
  return minpw + ((1 + (pw << 1) * (maxpw - minpw)) / 252);  // round to nearest
}

void write126(byte tens, byte i)
{
  servo[i].writeMicroseconds(rescalePulseWidth(tens, minpw[i], maxpw[i]));
}

void setpw(byte count) {
  for (byte i = 0; i < count; i++) {
    minpw[i] = rescalePulseWidth(tmin[i], MIN_PULSE_WIDTH, MAX_PULSE_WIDTH / 2);
    servo[i].attach(pin[i], minpw[i], maxpw[i] = rescalePulseWidth(tmax[i], minpw[i], MAX_PULSE_WIDTH));
    servo[i].writeMicroseconds(minpw[i]);  // initialize servo positions
  }  
}
...
void setup() {            // setup() code runs once
  ...
  // change minPulseWidth, maxPulseWidth and set 0-126 pulse width range
...
void loop() {
  if (0 < Serial.available()) {
    ...
    if (0x7F == received) {
      ...
      for (byte i = 0; i < active_PWMs; i++) {
        tmax[i] = tmax_defaults[i];
        tmin[i] = min_defaults[i];       // initial servo positions
      }
      setpw(active_PWMs);
      Serial.write(msg);
    }
      ...
      if (Lcount) {                         // LUT load in progress?
        ...
        if (Lcount <= Lidx) {
          if (1 == Lid)                     // tmax[]?
            setpw(active_PWMs);             // SimHub profile always loads tmin[] first
          LUT = NULL;
      ...
      if (0x5F == loading) {                // info_level?
        ...
        if (30 == addr) {                         // active PWM count
          ...
          if (active_PWMs != received) {
            ...
            else for (byte i = received - 1; i < active_PWMs; i++)
              servo[i].detach();
            Serial.write("active_PWMs: ");
            Serial.print(active_PWMs = received);
            col += 15;
          }
        }
          ...
          if (126 <= received || 1 > received) {  // clipping
            digitalWrite(LED, LOW);  // illuminate LED
            if (1 == info_level) {
              Serial.print(addr);
              col++;
